...
enemy_image = pygame.image.load("nano_coding_pygame_basic/enemy.png")
enemy_size = enemy_image.get_rect().size # 이미지의 크기를 넣기
...
# 적 리스트
enemy_list = []
enemy_list.append(Enemy(enemy_width, enemy_height,  screen_width - enemy_width, 0, randint(10, 15)))
...
while running:
    ...
    for enemy in enemy_list:
        ...
        if enemy.pos_y > screen_heigth + enemy.height :
    ...
    for enemy in enemy_list:
        screen.blit(enemy_image, (enemy.pos_x, enemy.pos_y))              # 적 그리기

    # 타이머 집어 넣기
    ## 경과 시간 계산
    if elapsed_time < int((pygame.time.get_ticks() - start_ticks) / 1000) :
        elapsed_time = int((pygame.time.get_ticks() - start_ticks) / 1000) # 경과 시간은 ms 단위라서 s 단위로 변경
        enemy_list.append(Enemy(enemy_width, enemy_height,  randint(1, screen_width - enemy_width), 0, randint(10, 15)))
